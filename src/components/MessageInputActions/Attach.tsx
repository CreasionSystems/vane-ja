  ...
  const handleChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!embeddingModelProvider || !embeddingModel) {
        throw new Error(t('attach.needEmbeddingModel'));
      }
      ...
      if (!res.ok) {
        throw new Error(resData.message || t('attach.uploadFailed'));
      }

      if (!Array.isArray(resData.files)) {
        throw new Error(t('attach.invalidResponse'));
      }
      ...
      toast(err?.message || t('attach.uploadFailed'));
    } finally {
      setLoading(false);
      e.target.value = '';
    }
                  ...
                  <div className="flex flex-row items-center justify-between px-3 py-2">
                    ...
                    <div className="flex flex-row items-center space-x-4">
                      ...
                      >
                        ...
                        />
                        <Plus size={16} />
                        <p className="text-xs">{t('attach.add')}</p>
                      </button>
                      <button
                        onClick={() => {
                          setFiles([]);
                      ...
                      >
                        <Trash size={13} />
                        <p className="text-xs">{t('attach.clear')}</p>
                      </button>
                    </div>
                  </div>
