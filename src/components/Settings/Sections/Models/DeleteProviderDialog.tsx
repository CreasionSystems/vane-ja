...
const DeleteProvider = ({
  ...
  const handleDelete = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('models.connectionDeleted'));
    } catch (error) {
      console.error('Error deleting provider:', error);
      toast.error(t('models.deleteConnectionFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <button
        ...
        title={t('models.deleteConnection')}
      >
        <Trash2
          size={14}
              ...
              <DialogPanel className="w-full mx-4 lg:w-[600px] max-h-[85vh] flex flex-col border bg-light-primary dark:bg-dark-primary border-light-secondary dark:border-dark-secondary rounded-lg">
                <div className="px-6 pt-6 pb-4">
                  <h3 className="text-black/90 dark:text-white/90 font-medium">
                    {t('models.deleteConnection')}
                  </h3>
                </div>
                <div className="border-t border-light-200 dark:border-dark-200" />
                <div className="flex-1 overflow-y-auto px-6 py-4">
                  <p className="text-sm text-black/60 dark:text-white/60">
                    {t('models.deleteConnectionBody', {
